...
3. **加速度曲线**：在空中下落时加速度稳定在 $-9.81 \text{ m/s}^2$。在与地面碰撞的一瞬间，由于接触力（Contact Force）的作用，会产生一个超过 $1000 \text{ m/s}^2$ 的巨大正向脉冲加速度。由于此脉冲信号与重力加速度的量级相差悬殊，常规的线性坐标轴会导致重力加速度被压缩为一条几乎贴近 $0$ 刻度的横线。因此，我们在代码中**采用了对称对数刻度（`symlog`）**来绘制 Y 轴，从而可以清晰地同时展示自由落体阶段的 $-9.81 \text{ m/s}^2$ 重力加速度与碰撞瞬间的超大加速度脉冲。
